...
def test_audio_recog() -> (bool, int):
    ...
    log(__file__, f"Took {now_time() - start_time}s to load processor and model.")

    start_time = now_time()
    data = convert_sample_rate((load_file("src/audio/model_test.wav")), 16000)

    log(__file__, f"Took {now_time() - start_time}s to load and downsample the audio file.")

    start_time = now_time()
    features = processor(data, sampling_rate=16000, return_tensors="tf").input_features
    tokens = model.generate(features)

    log(__file__, f"Took {now_time() - start_time}s to process and generate the tokens for the audio file.")
    ...
    log(__file__, f"Finished audio recog. in {now_time() - overall_time}s. Status: {passed}", LogTypes.success)
